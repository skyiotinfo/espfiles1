...
// ── Realtime: track last sent string to avoid redundant serial sends ──
String lastSentUart = "";
...
void loop() {
  ...
  lcd.print(sdevice[3]); lcd.print(" ");
  lcd.print(sdevice[4]); lcd.print(" ");
  lcd.print(sdevice[5]); lcd.print(" ");
  ...
  // ── REALTIME FIX: only send serial when state actually changed ──
  if (uartData != lastSentUart) {
    Serial.println(uartData);
    lastSentUart = uartData;
  }

  // ── REALTIME FIX: reduced from 500ms to 50ms ──
  delay(50);
