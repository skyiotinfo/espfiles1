...
unsigned long lastRealtimeActivity = 0;
const unsigned long REALTIME_TIMEOUT = 30000;
unsigned long now;
unsigned long lastRealtimeConnect = 0;
unsigned long lastReconnectAttempt = 0;

#define REALTIME_RECONNECT_TIMEOUT 120000  // 2 minutes
#define RECONNECT_COOLDOWN         30000
...
void updateSupabaseMotorState(int state) {  
  realtime.loop();
  int code=db.update("pump_motor").eq("column", "state").doUpdate(JSON);
  delay(500);
  Serial.println("Supabase Update Code: " + code);
  db.urlQuery_reset();
  realtime.loop();   
}
...
void connectSupabase() {
  ...
  lastRealtimeConnect  = millis();   
  lastRealtimeActivity = millis();  
}


void maintainRealtime() {
  realtime.loop();
  ...
  if (!supabaseConnected &&
      millis() - lastRealtimeConnect  > REALTIME_RECONNECT_TIMEOUT &&
      millis() - lastRealtimeActivity > REALTIME_RECONNECT_TIMEOUT) {

    Serial.println("Realtime stalled → reconnecting");
    supabaseConnected = false;
    connectSupabase();
  }
}
