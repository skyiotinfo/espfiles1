...
// ════════════ NEW: Gateway board MAC ════════════
// Fill in with the MAC printed by gateway_board.ino's "Gateway MAC:" serial line.
uint8_t gatewayMac[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
struct Schedule {
  ...
  bool     active;
  bool     enabled;     // ════════════ NEW: app can disable a schedule without losing its config ════════════
};

Schedule schedules[3] = {
  {100, 20, 44, 100, false, true},
  {10,  20, 46, 100, false, true},
  {1,   20, 48, 40, false, true},
};
//
...
const unsigned long MANUAL_DURATION = 120000UL;
...
// ════════════ NEW: Gateway command protocol (must match gateway_board.ino) ════════════
enum GwCmd : uint8_t {
  CMD_SET_SIGNAL          = 1,
  CMD_STOP_ALL             = 2,
  CMD_RUN_MANUAL_SEQUENCE = 3,
  CMD_SET_SCHEDULE        = 4,
  CMD_GW_HELLO             = 5,
};
enum MotorMsg : uint8_t {
  MSG_STATUS = 1,
};
#pragma pack(push, 1)
struct GwPacket {
...
struct MotorStatusPacket {
  ...
  uint16_t lastAck;
  uint8_t  manualState;
  uint16_t manualSecsLeft;
  uint8_t  forceOffActive;
  uint32_t cmdAckSeq;
  uint32_t schAckSeq;
};
#pragma pack(pop)

uint32_t lastCmdAckSeq = 0;   // last manual-cmd seq we've applied (echoed to Gateway)
uint32_t lastSchAckSeq = 0;   // last schedule seq we've applied (echoed to Gateway)
unsigned long lastStatusPushMs = 0;
const unsigned long STATUS_PUSH_INTERVAL_MS = 3000UL; // periodic heartbeat status, on top of event-driven pushes

void pushStatusToGateway();        // forward decl, defined below
void handleGatewayPacket(uint8_t *data, uint8_t len);  // forward decl, defined below
...
void processAck(uint16_t ack) {
  ...
  if (currentSignal != 0) {
    ...
    if (ackIsAcceptable(currentSignal, ack)) {
      ...
      Serial.println("⚠️  All valves closed unexpectedly — stopping motor");
      stopMotorOnly();
...
void onReceive(uint8_t *mac, uint8_t *data, uint8_t len) {
  // ════════════ NEW: route by sender MAC — Gateway commands vs Valve1 ACKs ════════════
  if (memcmp(mac, gatewayMac, 6) == 0) {
    handleGatewayPacket(data, len);   // defined below
    return;
  }

  // ── Original Valve1 ACK path (unchanged) ──
  if (len < 2) return;
  if (ackQueueIdx >= 16) return;
...
// ════════════ NEW: Gateway command handling ════════════

void handleGatewayPacket(uint8_t *data, uint8_t len) {
  if (len < (int)sizeof(GwPacket)) return;
  GwPacket pkt;
  memcpy(&pkt, data, sizeof(GwPacket));

  Serial.printf("📥 Gateway cmd=%d payload=%u seq=%lu\n", pkt.cmd, pkt.payload, (unsigned long)pkt.seq);

  switch (pkt.cmd) {

    case CMD_SET_SIGNAL: {
      // Direct manual valve mask from app — treat like a manual override (cancels schedule timers via manualState use is optional)
      currentSignal = pkt.payload;
      sendSignal(currentSignal);
      lastCmdAckSeq = pkt.seq;
      break;
    }
    ...
    case CMD_RUN_MANUAL_SEQUENCE: {
      // Same as physical button press: V1 -> V2 -> V3, 120s each (reuses existing runManualSequence() state machine)
      if (manualState == MANUAL_IDLE) {
        Serial.println("📱 App-triggered manual sequence starting");
    ...
    case CMD_SET_SCHEDULE: {
      uint8_t  idx     = (pkt.payload >> 13) & 0x03;
      bool     en      = (pkt.payload & 0x8000) != 0;
      uint16_t mask    = pkt.payload & 0x1FFF;
      uint8_t  hour    = (pkt.value2 >> 24) & 0xFF;
      uint8_t  minu    = (pkt.value2 >> 16) & 0xFF;
      uint16_t durSec  = pkt.value2 & 0xFFFF;

      if (idx < 3) {
        schedules[idx].valveMask   = mask;
        schedules[idx].startHour   = hour;
        schedules[idx].startMin    = minu;
        schedules[idx].durationSec = durSec;
        schedules[idx].enabled     = en;
        // 'active' state intentionally untouched here — checkSchedules() re-evaluates each loop
        Serial.printf("📅 Schedule %d updated: mask=%03u %02d:%02d dur=%us enabled=%d\n",
                      idx, mask, hour, minu, durSec, en);
      }
      lastSchAckSeq = pkt.seq;
      break;
    }

    case CMD_GW_HELLO: {
      // Gateway just rebooted / wants a fresh status; reply immediately
      break;
    }
...
// ════════════ NEW: status push to Gateway ════════════

void pushStatusToGateway() {
  MotorStatusPacket pkt;
  pkt.msgType        = MSG_STATUS;
  pkt.motorOn         = motorRunning ? 1 : 0;
  pkt.currentSignal   = currentSignal;
  pkt.lastAck         = lastAck;
  pkt.manualState     = (uint8_t)manualState;
  if (manualState != MANUAL_IDLE && manualState != MANUAL_DONE) {
    long remaining = (long)(MANUAL_DURATION - (millis() - manualStepStart)) / 1000L;
    pkt.manualSecsLeft = (uint16_t)(remaining < 0 ? 0 : remaining);
  } else {
    pkt.manualSecsLeft = 0;
  }
  pkt.forceOffActive  = forceOffActive ? 1 : 0;
  pkt.cmdAckSeq       = lastCmdAckSeq;
  pkt.schAckSeq       = lastSchAckSeq;
...
void checkSchedules() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (!schedules[i].active && schedules[i].enabled && nowSec >= startSec && nowSec < stopSec) {
  ...
  switch (manualState) {
    ...
    case MANUAL_V1:
      if (elapsed >= MANUAL_DURATION) {
        Serial.println("Manual: V1 time done → motor OFF → closing V1");
        stopMotorOnly();
        delay(2000);
        currentSignal    = 0;
        switchPhase2Sent = false;
        manualStepStart  = millis();  // reset so SWITCH_TIMEOUT counts from here
    ...
    case MANUAL_V2:
      if (elapsed >= MANUAL_DURATION) {
        Serial.println("Manual: V2 time done → motor OFF → closing V2");
        stopMotorOnly();
        delay(2000);
        currentSignal    = 0;
        switchPhase2Sent = false;
...
void setup() {
  ...
  esp_now_add_peer(gatewayMac, ESP_NOW_ROLE_COMBO, 1, NULL, 0);   // ════════════ NEW ════════════
...
void loop() {
 ...
 // handleHeartbeat();
  ...
  // ════════════ NEW: periodic status heartbeat to Gateway ════════════
